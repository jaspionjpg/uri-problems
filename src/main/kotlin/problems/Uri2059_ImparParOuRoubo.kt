...
fun main(args: Array<String>) {
    ...
    val jogador1EscolheuPar = ler.nextInt() == 1

    val numeroJogador1 = ler.nextInt()
    val numeroJogador2 = ler.nextInt()

    val jogador1Roubou = ler.nextInt() == 1
    val jogador2Acusou = ler.nextInt() == 1

    val soma = numeroJogador1 + numeroJogador2

    var win = if ((soma % 2 == 0 && jogador1EscolheuPar) ||
        (soma % 2 == 1 && !jogador1EscolheuPar)
    ) 1
    else 2

    if ((jogador1Roubou && !jogador2Acusou) || (!jogador1Roubou && jogador2Acusou))
        win = 1
    else if (jogador1Roubou && jogador2Acusou)
        win = 2

    println("Jogador $win ganha!")
}

/*
URI Online Judge | 2059
Ímpar, Par ou Roubo
Por Marianne Linhares, UFCG BR Brazil
