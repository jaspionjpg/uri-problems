...
fun main(args: Array<String>) {
    ...
    while (true) {
        var bilhetesOriginais = ler.nextInt()
        val bilhetesTotais = ler.nextInt()
        if (bilhetesTotais == 0 && bilhetesOriginais == 0)
            break

        val bilhetes = IntArray(bilhetesOriginais)

        for (i in 0 until bilhetesTotais) {
            val bilhete = ler.nextInt()
            bilhetes[bilhete - 1]++
        }

        println(bilhetes.filter { i -> i > 1 }.size)
    }
}
