...
fun main(args: Array<String>) {
    ...
    var numeroTeste = 0
    while (true) {
        val numeroFrases = ler.nextLine().toInt()
        if (numeroFrases == 0)
            break
        ...
        val listaFrases = ArrayList<String>()
        var maiorLength = 0

        for (i in 0 until numeroFrases) {
            val fraseLidaETratada = ler.nextLine().replace(Regex("\\s+"), " ").trim()
            listaFrases.add(fraseLidaETratada)
            if (fraseLidaETratada.length > maiorLength)
                maiorLength = fraseLidaETratada.length
        }

        for (frase in listaFrases) {
            var pontosEmBrancos = ""
            for (i in frase.length until maiorLength)
                pontosEmBrancos += " "

            println(pontosEmBrancos + frase)
        }
    }
}

/*
URI Online Judge | 1278
Justificador II
