...
fun main(args: Array<String>) {
    ...
    while (true) {
        val mensagensColetadas = ler.nextInt()
        if (mensagensColetadas == 0)
            break

        val lista = mutableMapOf<Int, String>()

        for (mensagem in 0 until mensagensColetadas) {
            val planeta = ler.next()
            val anoChegou = ler.nextInt()
            val anoAteChegar = ler.nextInt()

            lista[anoChegou - anoAteChegar] = planeta
        }

        println(lista[lista.toSortedMap().firstKey()])
    }
}

/*
URI Online Judge | 1515
Hello Galaxy
