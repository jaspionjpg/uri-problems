...
fun main(args: Array<String>) {
    ...
    val numeroFrases: Int = ler.nextLine().toInt()
    for (i in 0 until numeroFrases) {
        val frase: String = ler.nextLine()
        val comecoFrase = frase.substring(0, frase.length / 2)
        val finalFrase = frase.substring(frase.length / 2)

        println(comecoFrase.reversed() + finalFrase.reversed())
    }
}
