...
fun main(args: Array<String>) {
    ...
    for (i in 0 until testes) {
        val hora = ler.nextInt()
        val minuto = ler.nextInt()
        val ocorrencia = if (ler.nextInt() == 1) "abriu" else "fechou"

        var tempo = "%02d".format(hora) + ":" + "%02d".format(minuto)
        println("$tempo - A porta $ocorrencia!")
    }
}
