...
fun main(args: Array<String>) {
    ...
    while (true) {
        val regioes = ler.nextInt()
        if (regioes == 0)
            break

        val listaCidades = BooleanArray(regioes) { true }

        listaCidades[0] = false

        var pulo = 1

        var lugaresSemEnergia = 1
        var lugaresPulados = 1
        var indiceAtual = 1

        var listaCidadesTeste = listaCidades.clone()
        while (true) {
            if (listaCidadesTeste[indiceAtual])
                if (lugaresPulados == pulo) {
                    listaCidadesTeste[indiceAtual] = false
                    lugaresPulados = 1
                    if (indiceAtual == 12)
                        if (lugaresSemEnergia == regioes - 1)
                            break
                        else {
                            pulo++
                            listaCidadesTeste = listaCidades.clone()
                            lugaresSemEnergia = 1
                            lugaresPulados = 1
                            indiceAtual = 1
                            continue
