...
fun main(args: Array<String>) {
    ...
    for (i in 1..numeroDeTestes) {
        ...
        val numeroElementos = ler.nextInt()

        var somaTeste = 0
        for (j in 0 until numeroElementos) {
            val texto = ler.next()

            for (indexCaractere in texto.indices) {
                val caractere = texto[indexCaractere]
                val valorPosicaoAlfabeto = alfabeto.indexOf(caractere)
                val valorPosicao = indexCaractere
                val valorElemento = j


                somaTeste += valorPosicaoAlfabeto + valorPosicao + valorElemento
            }
        }
        println(somaTeste)
    }
