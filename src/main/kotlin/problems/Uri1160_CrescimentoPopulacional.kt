...
fun main(args: Array<String>) {
    ...
    for (teste in 1..testes) {
        var populacaoA = ler.nextDouble()
        var populacaoB = ler.nextDouble()
        val crescimentoA = 1 + ler.nextDouble() / 100
        val crescimentoB = 1 + ler.nextDouble() / 100

        var anos = 0
        while (populacaoA <= populacaoB) {
            populacaoA = floor(populacaoA * crescimentoA)
            populacaoB = floor(populacaoB * crescimentoB)
            anos++
            if (anos > 100)
                break
        }
